...
export function LiveStats({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={`border-y border-violet/25 bg-void/80 ${compact ? "px-3 py-3" : "px-4 py-4 md:px-10"}`}>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] sm:grid sm:grid-cols-4 sm:overflow-visible lg:grid-cols-7 [&::-webkit-scrollbar]:hidden">
        {items.map((s) => (
