...
export function reviewTrade(p: PolicyIn): PolicyOut {
  ...
  if (read.n7 < 12) return { ok: false, reason: "Not enough history on this pair." };
  ...
  // Gold vs USD mean-reverts weakly. Demand more stretch.
  ...
  // QQQ vs SPY is a small-edge pair.
  ...
  // SOL trends. Modest fades vs USDC in a 7d trend lose.
  if (pair.id === "sol-usdc") {
  ...
  // After hours / weekend: tokenized equities diverge from cash. Gold/SOL may still trade.
  ...
  let score = Math.abs(read.z7) * 10 + Math.abs(read.z24) * 4;
  let clip = clipUsd;
  if (atr > 0.008 && (from === "SOL" || to === "SOL")) {
    clip *= 0.7;
    score -= 4;
  }
  if (session === "cash" && (pair.left === "SPYx" || pair.right === "SPYx" || pair.left === "QQQx" || pair.right === "QQQx")) {
  ...
  if (pair.id === "usdc-gldx" || pair.id === "sol-gldx") score -= 1;
