...
import { ensureCircle, postMessage } from "@/lib/circle/engine";
...
export async function POST(req: NextRequest) {
  if (!rateLimit(clientIp(req) + ":circmedia", 12, 60_000)) {
  ...
  const pinned = await pinBytes(buf, mime, "circle");
  if (!pinned?.url) {
  ...
  const posted = await mutateState((s) => {
    ...
    return postMessage(s.circle, { owner: pubkey, kind: "media", media: pinned.url, replyTo });
  });
